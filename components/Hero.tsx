...
const Hero: React.FC = () => {
  return (
    <section className="relative min-h-[90vh] md:min-h-screen flex items-center justify-center overflow-hidden hero-gradient text-brand-snow pt-20 md:pt-24">
      ...
      {/* 3D Rotating Globe - Better positioning for mobile */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[500px] aspect-square md:w-[900px] md:h-[900px] opacity-40 md:opacity-30 pointer-events-none transition-all duration-1000">
      ...
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-10 md:gap-16 items-center relative z-10 text-center lg:text-left">
        ...
        {/* Hero image hidden on small mobile, visible on tablet/desktop */}
        <div className="hidden lg:block relative group">
          <div className="relative w-full aspect-[4/5] max-w-lg mx-auto animate-float">
             <div className="absolute inset-0 bg-brand-teal/20 rounded-[48px] translate-x-8 translate-y-8 blur-sm transition-transform group-hover:translate-x-10"></div>
             <div className="absolute inset-0 bg-brand-red/10 rounded-[48px] -translate-x-8 -translate-y-8 blur-sm transition-transform group-hover:-translate-x-10"></div>
             <div className="relative rounded-[48px] overflow-hidden w-full h-full border border-white/10 shadow-[0_60px_120px_-20px_rgba(0,0,0,0.6)]">
               <img 
                 src="https://images.unsplash.com/photo-1494412651409-8963ce7935a7?auto=format&fit=crop&w=1200&q=80" 
                 alt="Professional Trade" 
                 className="object-cover w-full h-full brightness-75 group-hover:scale-105 transition-transform duration-1000"
               />
               <div className="absolute inset-0 bg-gradient-to-t from-brand-navy/90 via-brand-navy/20 to-transparent"></div>
               <div className="absolute bottom-12 left-12 right-12 transform group-hover:translate-y-[-8px] transition-transform text-left">
                 <p className="text-brand-teal font-black text-3xl tracking-tighter mb-1">ORIONTEL EXIM</p>
                 <p className="text-[11px] uppercase tracking-[0.5em] font-bold opacity-60">Logistics Excellence Hub</p>
                 <div className="w-12 h-1 bg-brand-red mt-4 rounded-full"></div>
               </div>
             </div>
          </div>
        </div>
